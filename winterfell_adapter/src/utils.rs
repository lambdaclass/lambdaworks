...
pub fn matrix_field2adapter(
    input: &[Vec<FieldElement<Stark252PrimeField>>],
) -> Vec<Vec<AdapterFieldElement>> {
    input.iter().map(|v| vec_field2adapter(v)).collect()
}

pub fn matrix_adapter2field(
    input: &[Vec<AdapterFieldElement>],
