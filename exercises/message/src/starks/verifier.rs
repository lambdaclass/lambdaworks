...
fn step_2_verify_claimed_composition_polynomial<F: IsFFTField, A: AIR<Field = F>>(
    ...
    // println!("Result: {}", composition_poly_claimed_ood_evaluation == composition_poly_ood_evaluation);
    ...
    FieldElement<F>: ByteConversion,
{
    ...
    let _next_layer_evaluation = &fri_decommitment.layers_evaluations[0];
            ...
            |result,
             (
                ...
                if k < fri_decommitment.layers_evaluations.len() - 1 {
                    let _next_layer_evaluation = &fri_decommitment.layers_evaluations[k + 1];
                    result & true
                } else {
