    ...
    FieldElement<F>: ByteConversion + Send + Sync,
{
    ...
    let round_3_result = round_3_evil(
        &air,
        &domain,
        &round_1_result,
        &z,
    ...
    // protocol on its own. Therefore we pass it the transcript
    // to simulate the interactions with the verifier.
    let round_4_result = round_4_evil(
        &air,
        &domain,
        &round_1_result,
        &round_2_result,
    ...
    FieldElement<F>: ByteConversion,
{
    ...
    println!(
        ...
        evil prover should pwn verifier. iota_0 = {}", iotas[0]);
    */
    let fri_layers_merkle_roots: Vec<_> = fri_layers
        .iter()
        .map(|layer| layer.merkle_tree.root)
