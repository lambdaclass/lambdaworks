...
pub fn point_double_projective(c: &mut Criterion) {
    ...
    println!("Starknet RS result x: {}", AffinePoint::from(&initial_point).x);
    println!("Starknet RS result y: {}", AffinePoint::from(&initial_point).y);
    ...
    println!("Lambdaworks result x: {:?}", test_lambda_result.to_affine().x());
    println!("Lambdaworks result y: {:?}", test_lambda_result.to_affine().y());
