...
impl MerkleTree {
    pub fn build(
        unhashed_leaves: &[impl ByteConversion],
        hasher: impl IsHasher,
    ) -> MerkleTree
    {
        let mut hashed_leaves = hash_leaves(unhashed_leaves, &hasher);
        ...
        let mut inner_nodes = vec![[0u8; 32] ; hashed_leaves.len() - 1];
    ...
    pub fn get_proof_by_pos(&self, pos: usize) -> Option<Proof> {
    ...
    fn create_proof(&self, merkle_path: Vec<[u8; 32]>) -> Option<Proof> {
