...
) -> (Vec<Polynomial<FieldElement<P::BaseField>>>, FieldElement<P::BaseField>)
where 
...
) -> (FieldElement<P::BaseField>, (Vec<FieldElement<P::BaseField>>, Vec<FieldElement<P::BaseField>>)) {
...
<P as IsPairing>::BaseField: IsPrimeField<RepresentativeType = UnsignedInteger<N>>,
{
  ...
  fn prove(
    ...
    debug_assert_eq!(pi_poly.evaluate(&x_challenge), FieldElement::<P::BaseField>::zero());
  ...
  fn verify(
    ...
    let bases = [
      vec![q_hat_com, batched_commitment, vk.g1],
      q_k_com,
    ]
    .concat();
    let zeta_z_com = msm(
    ...
    let e = P::compute_batch(&[(&pi, &(vk.tau_2 - (vk.g2.operate_with_self(x_challenge.representative())))), (&zeta_z_com, &vk.tau_N_max_sub_2_N)]).unwrap();
