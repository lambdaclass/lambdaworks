...
//type G2 = ShortWeierstrassProjectivePoint<BLS12377TwistCurve>;

fn rand_pairing_points(num: usize) -> (Vec<G1>, (Vec<u128>, Vec<u128>)) {
...
pub fn bls12377_elliptic_curve_benchmarks(c: &mut Criterion) {
    //TODO: find a better method of generating and representing benchmark data. This is very verbose and redunant.
    let input: Vec<(Vec<G1>, (Vec<u128>, Vec<u128>))> = [1, 10, 100, 1000, 10000, 100000, 1000000]
        .into_iter()
        .map(rand_pairing_points)
        .collect::<Vec<_>>();
    let operate_with_points = input.iter().fold(Vec::new(), |mut acc, (g1, (a, b))| {
        let g1s = g1
            .iter()
            .zip(b.iter())
            .map(|(g1, b)| g1.operate_with_self(*b))
            .collect::<Vec<_>>();
        //let g2s = g2.iter().zip(a.iter()).map(|(g2, a)| g2.operate_with_self(*a)).collect::<Vec<_>>();
        acc.push(g1s);
        acc
    });
    ...
    // Operate_with G2
    /*
    for (((_,g2), _), (_,g2_a)) in input.clone().into_iter().zip(operate_with_points.clone().into_iter()) {
        group.bench_with_input(format!("Operate_with_G2 {:?}", &g2.len()), &(g2, g2_a), |b, (g2, g2_a)| {
            b.iter(|| {
    ...
    */
    ...
    for (g1, (_, b)) in input.clone().into_iter() {
        group.bench_with_input(
    ...
    for ((_, g2), (_, a)) in input.clone().into_iter() {
        group.bench_with_input(format!("Operate_with_self_G2 {:?}", &g2.len()), &(g2, a), |b, (g2, a)| {
            b.iter(|| {
    ...
    */
    ...
    // Double G1
    for (g1, _) in input.clone().into_iter() {
    ...
    */
    ...
    for ((g1, _), _) in input.clone().into_iter() {
        group.bench_with_input(format!("Compress G1 point {:?}", &g1.len()), &g1, |b, g1| {
            b.iter(|| {
                for p in g1 {
    ...
    // Decompress_G1_point
    for ((g1, _), _) in input.clone().into_iter() {
        group.bench_with_input(format!("Decompress G1 Point {:?}", &g1.len()), &g1, |b, g1| {
            let g1_data = g1.into_iter().map(|g1| {
                //Eww
    ...
    for ((g1, g2),_) in input.clone().into_iter() {
        group.bench_with_input(format!("Ate Pairing {:?}", &g1.len()), &(g1, g2), |b, (g1, g2)| {
            b.iter(|| {
