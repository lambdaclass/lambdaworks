...
use lambdaworks_math::{
    fft::polynomial::FFTPoly, field::traits::RootsConfig,
...
pub fn poly_evaluate_fft(poly: &UnivariatePolynomial<Stark252PrimeField>) -> Vec<FE> {
    poly.evaluate_fft(black_box(1), black_box(None)).unwrap()
}

pub fn poly_interpolate_fft(evals: &[FE]) {
    UnivariatePolynomial::interpolate_fft(evals).unwrap();
}
