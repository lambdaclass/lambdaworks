...
fn miller<E: IsShortWeierstrass>(
    ...
    while order_r > E::UIntOrders::from(0) {
        bs.insert(0, order_r & E::UIntOrders::from(1));
        order_r = order_r >> 1;
    }
...
#[allow(unused)]
pub fn weil_pairing<E: IsShortWeierstrass>(
    p: &ShortWeierstrassProjectivePoint<E>,
    q: &ShortWeierstrassProjectivePoint<E>,
) -> FieldElement<E::BaseField> {
    if p.is_neutral_element() || q.is_neutral_element() || p == q {
        ...
        let numerator = miller(p, q);
        let denominator = miller(q, p);
...
/// See "Pairing for beginners" from Craig Costello, page 79.
pub fn tate_pairing<E: IsShortWeierstrass>(
    p: &ShortWeierstrassProjectivePoint<E>,
    q: &ShortWeierstrassProjectivePoint<E>,
) -> FieldElement<E::BaseField> {
    if p.is_neutral_element() || q.is_neutral_element() || p == q {
        ...
        let mut base = miller(p, q);
        let bit_representation_exponent = E::target_normalization_power();
