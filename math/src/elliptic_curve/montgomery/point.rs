...
#[derive(Clone, Debug)]
pub struct MontgomeryJacobianPoint<E: IsEllipticCurve>(JacobianPoint<E>);

impl<E: IsEllipticCurve> MontgomeryJacobianPoint<E> {
    /// Creates an elliptic curve point giving the projective [x: y: z] coordinates.
    pub fn new(value: [FieldElement<E::BaseField>; 3]) -> Self {
...
impl<E: IsEllipticCurve> PartialEq for MontgomeryJacobianPoint<E> {
    fn eq(&self, other: &Self) -> bool {
        self.0 == other.0
    }
}

impl<E: IsMontgomery> FromAffine<E::BaseField> for MontgomeryJacobianPoint<E> {
    fn from_affine(
        x: FieldElement<E::BaseField>,
        y: FieldElement<E::BaseField>,
    ) -> Result<Self, crate::elliptic_curve::traits::EllipticCurveError> {
        if E::defining_equation(&x, &y) != FieldElement::zero() {
            ...
            let coordinates = [x, y, FieldElement::one()];
            Ok(MontgomeryJacobianPoint::new(coordinates))
        }
    }
}

impl<E: IsEllipticCurve> Eq for MontgomeryJacobianPoint<E> {}

impl<E: IsMontgomery> IsGroup for MontgomeryJacobianPoint<E> {
    /// The point at infinity.
    fn neutral_element() -> Self {
        Self::new([
            FieldElement::zero(),
