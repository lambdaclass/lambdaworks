...
pub trait Term<F: IsField>: Clone + Debug + Send + Sync {
    ...
    /// Evaluates `self` to a FieldElement<F> at the point `p` with all variables represented.
    fn evaluate(&self, p: &[FieldElement<F>]) -> FieldElement<F>;

    /// Evaluates 'self' to Self at the point 'p' with some not all variables represented.
