...
/// - λ_1 = (0 - 2) / (1 - 2) = 2
/// - λ_2 = (0 - 1) / (2 - 1) = -1
fn compute_lagrange_coefficient(identifier: Identifier) -> FE {
    match identifier {
        1 => FE::from(2u64),
        2 => -FE::one(),
        _ => panic!("Invalid identifier for 2-of-2 scheme"),
    }
}
...
fn compute_challenge(combined_r: &CurvePoint, group_public_key: &CurvePoint, message: &str) -> FE {
    let mut hasher = Keccak256::new();
...
pub fn sign_round2(
    ...
    let binding_factors: Vec<(Identifier, FE)> = all_commitments
        .iter()
        .map(|c| {
    ...
    let combined_r = compute_group_commitment(all_commitments, &binding_factors);
    ...
    let rho_i = binding_factors
        ...
        .map(|(_, rho)| rho)
        .expect("Missing own binding factor");
    ...
    let z_i = &nonces.hiding + &(&nonces.binding * rho_i) + &(&lambda_i * &key_share.secret_share * &c);
...
pub fn aggregate_signature(
    ...
    let binding_factors: Vec<(Identifier, FE)> = all_commitments
        ...
        .map(|c| {
            let rho =
                compute_binding_factor(group_public_key, all_commitments, message, c.identifier);
            (c.identifier, rho)
        })
        .collect();

    // Compute combined nonce point R
    let r = compute_group_commitment(all_commitments, &binding_factors);
...
mod tests {
    ...
    #[test]
    fn test_2_of_2_frost_signature() {
        let (share1, share2) = keygen();
