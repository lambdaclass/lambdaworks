...
pub fn generate_evaluation_key(
    ...
    EvaluationKey {
    g1_vk: v_mid.iter().map(|vk| g1.operate_with_self((rv * vk.evaluate(&s)).representative())).collect(),
    g1_wk: w_mid.iter().map(|wk| g1.operate_with_self((rw * wk.evaluate(&s)).representative())).collect(),
    g2_wk: w_mid.iter().map(|wk| g2.operate_with_self((rw * wk.evaluate(&s)).representative())).collect(),
    g1_yk: y_mid.iter().map(|yk| g1.operate_with_self((ry * yk.evaluate(&s)).representative())).collect(),
    g1_alpha_vk: v_mid.iter().map(|vk| g1.operate_with_self((rv * alpha_v * vk.evaluate(&s)).representative())).collect(),
    g1_alpha_wk: w_mid.iter().map(|wk| g1.operate_with_self((rw * alpha_w * wk.evaluate(&s)).representative())).collect(),
    g1_alpha_yk: y_mid.iter().map(|yk| g1.operate_with_self((ry * alpha_y * yk.evaluate(&s)).representative())).collect(),
