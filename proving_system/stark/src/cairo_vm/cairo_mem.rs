    ...
    fn from_bytes_le(bytes: &[u8]) -> Result<Self,CairoImportError> {
        ...
        for i in 0..num_rows {
            cells.push(CairoMemoryCell {
                address: u64::from_le_bytes(bytes[i * ROW_SIZE..i * ROW_SIZE + 8].try_into().unwrap()),
                value: U256::from_bytes_le(bytes[i * ROW_SIZE + 8..i * ROW_SIZE + 40].try_into().unwrap()).unwrap(),
            })
        }
