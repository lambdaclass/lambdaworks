...
impl<F: IsField, CS: IsCommitmentScheme<F>> Verifier<F, CS> {
    ...
    pub fn new(commitment_scheme: CS) -> Self {
        Self {commitment_scheme: commitment_scheme, phantom: PhantomData}
    }
        ...
        FieldElement<F>: ByteConversion
    {
        ...
        let verify = p_zeta - (zh_zeta * &p.t_zeta);
...
mod tests {
    ...
    fn test_verifier() {
        ...
        let prover = Prover::new(kzg.clone(), ORDER_4_ROOT_UNITY, ORDER_R_MINUS_1_ROOT_UNITY);
        let proof = prover.prove(&test_circuit, &public_input, &common_preprocesed_input);
        let verifier = Verifier::new(kzg.clone());
        assert!(verifier.verify(&proof, &test_circuit, &public_input, &common_preprocesed_input));
    }
}