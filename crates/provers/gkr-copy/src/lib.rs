    ...
    <F as IsField>::BaseType: Send + Sync,
{
        ...
        <F as IsField>::BaseType: Send + Sync + Copy,
    {
        ...
        for val in partial_point.iter() {
            add_i = add_i.fix_first_variable(val);
            mul_i = mul_i.fix_first_variable(val);
        }

        for val in b_partial.iter() {
            w_b = w_b.fix_first_variable(val);
        }

        for val in c_partial.iter() {
            w_c = w_c.fix_first_variable(val);
        }
...
mod tests {
    ...
    fn test_gkr_complete_verification() {
        ...
        // Generate proof
        let proof = gkr_prove(&circuit, &input).expect("Proof generation failed");
