...
/// Run the FRI commit phase with **deterministic** folding challenges.
///
/// Identical to [`fri_commit`] except that folding challenges come from the
/// `challenges` slice instead of being sampled from the transcript.  Merkle
/// roots are still absorbed into the transcript (for binding), and the final
/// constant value is still appended.
///
/// This is used for the Lagrange column `C_t(X)`: when folded with the GKR
/// evaluation point coordinates `(t_{n-1}, …, t_0)`, each fold eliminates one
/// variable of the eq polynomial, so the final value must equal 1.
pub fn fri_commit_deterministic<F, T>(
    ...
    T: IsTranscript<F>,
{
    ...
    loop {
        ...
        // Absorb Merkle root (for binding — verifier will replay this)
        ...
        // Use predetermined challenge instead of sampling from transcript
        if challenge_idx >= challenges.len() {
            return Err(FriError::InvalidConfig(format!(
                "ran out of deterministic challenges at layer {challenge_idx} \
                 (provided {} challenges but polynomial needs more folding steps)",
                challenges.len()
            )));
        }
        let beta = challenges[challenge_idx].clone();
        challenge_idx += 1;
