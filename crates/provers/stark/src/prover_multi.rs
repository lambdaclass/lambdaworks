...
use crate::fri;
use crate::proof::stark::{DeepPolynomialOpenings, MultiTraceStarkProof, PolynomialOpenings};
...
pub trait IsStarkProver<A: AIR> {
    ...
    /// Generates a STARK proof for the trace `main_trace` with public inputs `pub_inputs`.
    /// Warning: the transcript must be safely initializated before passing it to this method.
    fn multi_trace_prove(
        trace_1: &mut TraceTable<A::Field, A::FieldExtension>,
        trace_2: &mut TraceTable<A::Field, A::FieldExtension>,
        pub_inputs_1: &A::PublicInputs,
        pub_inputs_2: &A::PublicInputs,
        proof_options: &ProofOptions,
        mut transcript: impl IsTranscript<A::FieldExtension>,
    ) -> Result<MultiTraceStarkProof<A::Field, A::FieldExtension>, ProvingError>
    where
        A: Send + Sync,
        FieldElement<A::Field>: AsBytes + Send + Sync,
        A::FieldExtension: IsFFTField,
        FieldElement<A::FieldExtension>: AsBytes + Send + Sync,
    {
        ...
        let air_1 = A::new(trace_1.num_rows(), pub_inputs_1, proof_options);
        let air_2 = A::new(trace_2.num_rows(), pub_inputs_2, proof_options);
        ...
        validate_trace(
            &air_1,
            &round_1_result_1.main.trace_polys,
            round_1_result_1
                .aux
                .as_ref()
                .map(|a| &a.trace_polys)
                .unwrap_or(&vec![]),
            &domain,
            &round_1_result_1.rap_challenges,
        );
        #[cfg(debug_assertions)]
        validate_trace(
            &air_2,
            &round_1_result_2.main.trace_polys,
            round_1_result_2
                .aux
                .as_ref()
                .map(|a| &a.trace_polys)
                .unwrap_or(&vec![]),
            &domain,
            &round_1_result_2.rap_challenges,
        );
        ...
        let num_boundary_constraints_1 = air_1
            .boundary_constraints(&round_1_result_1.rap_challenges)
            .constraints
            .len();
        let num_boundary_constraints_2 = air_2
            .boundary_constraints(&round_1_result_2.rap_challenges)
            .constraints
            .len();
        ...
        let mut coefficients_1: Vec<_> =
            core::iter::successors(Some(FieldElement::one()), |x| Some(x * &beta_1))
                .take(num_boundary_constraints_1 + num_transition_constraints_1)
                .collect();
        let mut coefficients_2: Vec<_> =
            core::iter::successors(Some(FieldElement::one()), |x| Some(x * &beta_2))
                .take(num_boundary_constraints_2 + num_transition_constraints_2)
                .collect();

        let transition_coefficients_1: Vec<_> = coefficients_1
            .drain(..num_transition_constraints_1)
            .collect();
        let transition_coefficients_2: Vec<_> = coefficients_2
            .drain(..num_transition_constraints_2)
            .collect();
        let boundary_coefficients_1 = coefficients_1;
        let boundary_coefficients_2 = coefficients_2;

        let round_2_result_1 = Self::round_2_compute_composition_polynomial(
            &air_1,
            &domain,
            &round_1_result_1,
            &transition_coefficients_1,
            &boundary_coefficients_1,
        )?;

        let round_2_result_2 = Self::round_2_compute_composition_polynomial(
            &air_2,
            &domain,
            &round_1_result_2,
            &transition_coefficients_2,
            &boundary_coefficients_2,
        )?;

        // >>>> Send commitments: [H₁], [H₂]
        transcript.append_bytes(&round_2_result_1.composition_poly_root);
        transcript.append_bytes(&round_2_result_2.composition_poly_root);
        ...
        #[cfg(feature = "instruments")]
        let timer3 = Instant::now();
        // TODO: later make it compatible with different domains
        // <<<< Receive challenge: z
        let z = transcript.sample_z_ood(
            &domain.lde_roots_of_unity_coset,
            &domain.trace_roots_of_unity,
        ...
        let round_3_result_1 = Self::round_3_evaluate_polynomials_in_out_of_domain_element(
            &air_1,
            &domain,
            &round_1_result_1,
            &round_2_result_1,
        ...
        let round_3_result_2 = Self::round_3_evaluate_polynomials_in_out_of_domain_element(
            &air_2,
            &domain,
            &round_1_result_2,
            &round_2_result_2,
        ...
        // >>>> Send values: tⱼ(zgᵏ)
        let trace_ood_evaluations_columns_1 = round_3_result_1.trace_ood_evaluations.columns();
        ...
        let trace_ood_evaluations_columns_2 = round_3_result_2.trace_ood_evaluations.columns();
        ...
        // >>>> Send values: Hᵢ(z^N)
        for element_1 in round_3_result_1
            .composition_poly_parts_ood_evaluation
            .iter()
        {
        ...
        for element_2 in round_3_result_2
            .composition_poly_parts_ood_evaluation
            .iter()
        {
        ...
        // protocol on its own. Therefore we pass it the transcript
        // to simulate the interactions with the verifier.
        let round_4_result_1 = Self::round_4_compute_and_run_fri_on_the_deep_composition_polynomial(
            &air_1,
            &domain,
            &round_1_result_1,
            &round_2_result_1,
        ...
        let round_4_result_2 = Self::round_4_compute_and_run_fri_on_the_deep_composition_polynomial(
            &air_2,
            &domain,
            &round_1_result_2,
            &round_2_result_2,
        ...
        Ok(MultiTraceStarkProof::<A::Field, A::FieldExtension> {
            // [t]
            lde_trace_main_merkle_root_1: round_1_result_1.main.lde_trace_merkle_root,
            lde_trace_main_merkle_root_2: round_1_result_2.main.lde_trace_merkle_root,

            // [t]
            lde_trace_aux_merkle_root_1: round_1_result_1.aux.map(|x| x.lde_trace_merkle_root),
            lde_trace_aux_merkle_root_2: round_1_result_2.aux.map(|x| x.lde_trace_merkle_root),

            // tⱼ(zgᵏ)
            trace_ood_evaluations_1: round_3_result_1.trace_ood_evaluations,
            trace_ood_evaluations_2: round_3_result_2.trace_ood_evaluations,
            // [H₁] and [H₂]
            composition_poly_root_1: round_2_result_1.composition_poly_root,
            composition_poly_root_2: round_2_result_2.composition_poly_root,

            // Hᵢ(z^N)
            composition_poly_parts_ood_evaluation_1: round_3_result_1
                .composition_poly_parts_ood_evaluation,
            composition_poly_parts_ood_evaluation_2: round_3_result_2
                .composition_poly_parts_ood_evaluation,
            // [pₖ]
            fri_layers_merkle_roots_1: round_4_result_1.fri_layers_merkle_roots,
            fri_layers_merkle_roots_2: round_4_result_2.fri_layers_merkle_roots,

            // pₙ
            fri_last_value_1: round_4_result_1.fri_last_value,
            fri_last_value_2: round_4_result_2.fri_last_value,

            // Open(p₀(D₀), 𝜐ₛ), Open(pₖ(Dₖ), −𝜐ₛ^(2ᵏ))
            query_list_1: round_4_result_1.query_list,
            query_list_2: round_4_result_2.query_list,

            // Open(H₁(D_LDE, 𝜐₀), Open(H₂(D_LDE, 𝜐₀), Open(tⱼ(D_LDE), 𝜐₀)
            // Open(H₁(D_LDE, -𝜐ᵢ), Open(H₂(D_LDE, -𝜐ᵢ), Open(tⱼ(D_LDE), -𝜐ᵢ)
            deep_poly_openings_1: round_4_result_1.deep_poly_openings,
            deep_poly_openings_2: round_4_result_2.deep_poly_openings,

            // nonce obtained from grinding
            nonce: round_4_result_1.nonce,

            trace_length: air_1.trace_length(),
        })
    }
}
