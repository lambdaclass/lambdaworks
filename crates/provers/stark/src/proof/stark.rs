...
{
    pub proof: MultiTableProof,
    pub proof_sym: MultiTableProof,
...
//         authentication_paths: &[&MultiTableProof],
//         leaf_indexes: &[usize],
//     ) -> Vec<Vec<[u8; 32]>> {
//         debug_assert_eq!(leaf_indexes.len(), authentication_paths.len());
//         let mut merkle_tree: HashMap<(usize, usize), Vec<[u8;32]>> = HashMap::new();
...
//                 merkle_tree.insert((tree_level, node_index ^ 1), node.clone());
//                 node_index >>= 1;
//             }
//         }
