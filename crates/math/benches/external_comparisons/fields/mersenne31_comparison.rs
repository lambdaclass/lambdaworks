//! External comparison benchmarks: Lambdaworks vs Plonky3 (Mersenne31 field)
//!
//! Compares:
...
pub fn bench_lambdaworks(c: &mut Criterion) {
    let mut group = c.benchmark_group("Mersenne31 Lambdaworks");
...
pub fn bench_plonky3(c: &mut Criterion) {
    let mut group = c.benchmark_group("Mersenne31 Plonky3");

    let mut rng = StdRng::seed_from_u64(SEED);

    for size in SIZES {
        let values: Vec<P3Mersenne31> = (0..size)
