...
pub fn bn_254_elliptic_curve_benchmarks(c: &mut Criterion) {
    ...
    for size in [10, 50, 100, 200].iter() {
        ...
        batch_group.bench_function(
            ...
            |bencher| {
                bencher.iter(|| {
                    black_box(G1::batch_to_affine(black_box(&points)))
                });
            },
        );
    ...
    for size in [10, 50, 100, 200].iter() {
        ...
        batch_group.bench_function(
            ...
            |bencher| {
                bencher.iter(|| {
                    black_box(lambdaworks_math::elliptic_curve::batch::batch_add_sw::<BN254Curve>(black_box(&affine_points)))
                });
            },
        );
    ...
    for size in [10, 50, 100, 200].iter() {
        ...
        batch_group.bench_function(
            ...
            |bencher| {
                bencher.iter(|| {
                    black_box(lambdaworks_math::elliptic_curve::batch::batch_normalize_jacobian::<BN254Curve>(black_box(&jac_points)))
                });
            },
        );
        ...
        // batch_add_jacobian
        let jac_affine_points: Vec<ShortWeierstrassJacobianPoint<BN254Curve>> = jac_points.iter().map(|p| p.to_affine()).collect();

        batch_group.bench_function(
            format!("batch_add_jacobian/{}", size),
            |bencher| {
                bencher.iter(|| {
                    black_box(lambdaworks_math::elliptic_curve::batch::batch_add_jacobian::<BN254Curve>(black_box(&jac_affine_points)))
                });
            },
        );
