...
impl IsField for Degree2ExtensionField {
    ...
    /// Returns the multiplication of `a` and `b` using Karatsuba:
    /// (a0 + a1 * u) * (b0 + b1 * u) = a0*b0 - a1*b1 + (z - a0b0 - a1b1) * u
    /// where z = (a0 + a1) * (b0 + b1) and u^2 = -1.
    /// Uses 3 base field multiplications instead of 4.
    #[inline]
    ...
    /// This uses the equality `(a0 + a1 * u) * (a0 - a1 * u) = a0.pow(2) + a1.pow(2)` (since u^2 = -1)
    #[inline]
