//! Point compression for Vesta using SEC1 encoding.
//!
//! Compressed format: 33 bytes (0x02/0x03 prefix + 32 bytes x-coordinate)

use super::curve::VestaCurve;
use crate::{
    elliptic_curve::{
        short_weierstrass::{
...
type Point = ShortWeierstrassProjectivePoint<VestaCurve>;
type FE = FieldElement<Vesta255PrimeField>;

/// Compress a Vesta point to 33 bytes (SEC1 format).
#[cfg(feature = "alloc")]
...
/// Decompress a SEC1-encoded Vesta point from 33 bytes.
pub fn decompress_point(input: &[u8]) -> Result<Point, ByteConversionError> {
    ...
    // y² = x³ + 5
    let y_squared = x.pow(3_u16) + VestaCurve::b();
