// 256-bit prime field element arithmetic for Metal shaders.
//
...
//
// Uses Montgomery representation for efficient modular multiplication.
// All field elements are stored in Montgomery form: a_mont = a * R mod N
// where R = 2^256.
...
/// Template-based 256-bit prime field element.
///
/// Template parameters define the field:
/// - N_0..N_3: The prime modulus N (little-endian limbs)
/// - R_SQUARED_0..R_SQUARED_3: R^2 mod N for Montgomery conversion
/// - N_PRIME_0..N_PRIME_3: -N^(-1) mod R for Montgomery reduction
template <
...
public:
    ...
    /// Field inversion using addition chain method.
...
private:
    ...
    // R_SUB_N = 2^256 - N (for overflow handling)
    ...
    /// Repeated squaring helper: base^(2^N_ACC)
...
#pragma unroll
    ...
    /// Modular addition: (lhs + rhs) mod N
    /// Returns value in range [0, N)
    inline u256 add(const u256 lhs, const u256 rhs) const
    {
        u256 addition = (lhs + rhs);
        u256 res = addition;
        // Branchless conditional subtraction for GPU efficiency
        return res - u256(addition >= N) * N + u256(addition < lhs) * R_SUB_N;
    }

    /// Modular subtraction: (lhs - rhs) mod N
    /// Assumes rhs in range [0, N)
    inline u256 sub(const u256 lhs, const u256 rhs) const
    {
        return add(lhs, ((u256)N) - rhs);
    }

    /// Montgomery multiplication: (lhs * rhs) mod N
    ///
    /// Inputs must be in Montgomery form and in range [0, N).
    /// Uses REDC algorithm for efficient modular reduction.
    ///
    /// Reference:
