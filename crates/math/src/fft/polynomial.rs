...
#[cfg(feature = "cuda")]
use crate::fft::gpu::cuda::polynomial::{
    evaluate_fft_cuda, evaluate_fft_ext_cuda, interpolate_fft_cuda, interpolate_fft_ext_cuda,
};
#[cfg(feature = "cuda")]
...
/// SAFETY: Reinterprets a slice of `A` as a slice of `B`. Caller must ensure that
/// `A` and `B` have identical memory layouts (verified via `type_name` before calling).
#[cfg(feature = "cuda")]
unsafe fn cast_slice<A, B>(s: &[A]) -> &[B] {
    core::slice::from_raw_parts(s.as_ptr().cast(), s.len())
}

/// SAFETY: Reinterprets a `Vec<A>` as `Vec<B>` without copying. Caller must ensure that
/// `A` and `B` have identical memory layouts (verified via `type_name` before calling).
#[cfg(feature = "cuda")]
unsafe fn cast_vec<A, B>(v: Vec<A>) -> Vec<B> {
    let mut v = core::mem::ManuallyDrop::new(v);
    Vec::from_raw_parts(v.as_mut_ptr().cast(), v.len(), v.capacity())
}

/// Attempts CUDA extension field FFT evaluation for Goldilocks extensions.
/// Dispatches to the correct extension kernel by comparing `type_name` values.
///
/// SAFETY: The pointer casts are safe because we verify the concrete type via `type_name`
/// before casting. Both sides of each comparison are generated by the same compiler, so the
/// format is always consistent. `FieldElement<E>` is a plain struct wrapping `E::BaseType`,
/// and when `E` is the matched extension type, the memory layouts are identical.
#[cfg(feature = "cuda")]
fn try_evaluate_fft_ext_cuda<E: IsField>(
    coeffs: &[FieldElement<E>],
) -> Option<Result<Vec<FieldElement<E>>, FFTError>> {
    let e_name = core::any::type_name::<E>();
    let fp2_name = core::any::type_name::<Degree2GoldilocksExtensionField>();
    let fp3_name = core::any::type_name::<Degree3GoldilocksExtensionField>();

    if e_name == fp2_name {
        let ext_coeffs: &[FieldElement<Degree2GoldilocksExtensionField>] =
            unsafe { cast_slice(coeffs) };
        let result =
            evaluate_fft_ext_cuda::<Goldilocks64Field, Degree2GoldilocksExtensionField>(ext_coeffs);
        Some(match result {
            Ok(res) => Ok(unsafe { cast_vec(res) }),
            Err(e) => Err(e.into()),
        })
    } else if e_name == fp3_name {
        let ext_coeffs: &[FieldElement<Degree3GoldilocksExtensionField>] =
            unsafe { cast_slice(coeffs) };
        let result =
            evaluate_fft_ext_cuda::<Goldilocks64Field, Degree3GoldilocksExtensionField>(ext_coeffs);
        Some(match result {
            Ok(res) => Ok(unsafe { cast_vec(res) }),
            Err(e) => Err(e.into()),
        })
...
/// Attempts CUDA extension field FFT interpolation for Goldilocks extensions.
#[cfg(feature = "cuda")]
fn try_interpolate_fft_ext_cuda<E: IsField>(
    ...
    if e_name == fp2_name {
        let ext_evals: &[FieldElement<Degree2GoldilocksExtensionField>] =
            unsafe { cast_slice(fft_evals) };
        let result = interpolate_fft_ext_cuda::<Goldilocks64Field, Degree2GoldilocksExtensionField>(
            ext_evals,
        );
        Some(match result {
            Ok(poly) => {
                let ext_coeffs: &[FieldElement<E>] = unsafe { cast_slice(poly.coefficients()) };
                Ok(Polynomial::new(ext_coeffs))
            }
            Err(e) => Err(e),
        })
    } else if e_name == fp3_name {
        let ext_evals: &[FieldElement<Degree3GoldilocksExtensionField>] =
            unsafe { cast_slice(fft_evals) };
        let result = interpolate_fft_ext_cuda::<Goldilocks64Field, Degree3GoldilocksExtensionField>(
            ext_evals,
        );
        Some(match result {
            Ok(poly) => {
...
impl<E: IsField> Polynomial<FieldElement<E>> {
    ...
    pub fn evaluate_fft<F: IsFFTField + IsSubFieldOf<E>>(
        ...
        #[cfg(feature = "cuda")]
        {
            // When F = E (base field case), coefficients can be used directly with CUDA FFT.
            if (F::field_name() == "stark256" || F::field_name() == "goldilocks")
                && core::any::type_name::<F>() == core::any::type_name::<E>()
            {
                // SAFETY: F and E are the same type (verified via type_name).
                let f_coeffs: &[FieldElement<F>] = unsafe { cast_slice(&coeffs) };
                let result = evaluate_fft_cuda(f_coeffs)?;
                return Ok(unsafe { cast_vec(result) });
            }

            // Try extension field CUDA path for known Goldilocks extension types.
            if F::field_name() == "goldilocks" {
                if let Some(result) = try_evaluate_fft_ext_cuda(&coeffs) {
                    return result;
                }
            }
    ...
    pub fn interpolate_fft<F: IsFFTField + IsSubFieldOf<E>>(
        ...
        #[cfg(feature = "cuda")]
        {
            // When F = E (base field case), evaluations can be used directly with CUDA IFFT.
            if (F::field_name() == "stark256" || F::field_name() == "goldilocks")
                && core::any::type_name::<F>() == core::any::type_name::<E>()
            {
                // SAFETY: F and E are the same type (verified via type_name).
                let f_evals: &[FieldElement<F>] = unsafe { cast_slice(fft_evals) };
                let poly = interpolate_fft_cuda(f_evals)?;
                let ext_coeffs: &[FieldElement<E>] = unsafe { cast_slice(poly.coefficients()) };
                return Ok(Polynomial::new(ext_coeffs));
            }

            // Try extension field CUDA path for known Goldilocks extension types.
            if F::field_name() == "goldilocks" {
                if let Some(result) = try_interpolate_fft_ext_cuda(fft_evals) {
                    return result;
                }
            }
