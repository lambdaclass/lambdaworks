...
mod tests {
    ...
    #[test]
    fn line_vanishes_in_vanishing_points(){
        let first_vanish_point = CirclePoint::GENERATOR * 3;
        let second_vanish_point = CirclePoint::GENERATOR * 5;
        assert_eq!(line(&first_vanish_point, &first_vanish_point, &second_vanish_point), FE::zero());
        assert_eq!(line(&second_vanish_point, &first_vanish_point, &second_vanish_point), FE::zero());
    }

    #[test]
    fn interpolant_takes_the_corresponding_values(){
        let point_1 = CirclePoint::GENERATOR * 4;
        let point_2 = CirclePoint::GENERATOR * 9;
        let value_1 = FE::from(5);
        let value_2 = FE::from(3);
        assert_eq!(interpolant(&point_1, &point_2, value_1, value_2, &point_1), value_1);
        assert_eq!(interpolant(&point_1, &point_2, value_1, value_2, &point_2), value_2);
